...
export default function projectComponent(props) {
    ...
    const submit = (e) => {
        e.preventDefault();
        if(data.value != "" && data.value != props.attribute.value){
            post(route("customization.attribute"), {
                preserveScroll: true,
    ...
    return (
        <div className="flex items-center mb-5">
            ...
            <label htmlFor="value" className="my-2 block text font-bold text-gray-900 dark:text-white">{props.attribute.name}</label>
                <input
                    type="text"
                    name="value"
                    value={data.value}
